...
def vae_autoencoder_reduction(X, n_components):
    try:
        ...
        plt.legend()
        plt.savefig(f'/home/ousmane/results/Plot_loss_curve_gaussian_vae.png')
        plt.show()
...
def cluster_and_evaluate(X, y, n_clusters):
    try:
        model = GaussianMixture(n_components=n_clusters, random_state=42, covariance_type = "tied")
...
def main():
    ...
    mean_ari_scores = {key: np.mean([score for score in scores if score is not None]) if any(score is not None for score in scores) else None for key, scores in aggregated_results.items()}
    ...
    fig.add_trace(go.Bar(
       ...
       marker_color='orange',
       name='No Reduction',
       width=bar_width  # Appliquer la nouvelle largeur
    ))
    ...
    for i, method in enumerate(reduction_methods):
        ...
        fig.add_trace(go.Bar(
            x=x_method, 
            y=[mean_ari_scores[(method, n_components)] if mean_ari_scores[(method, n_components)] is not None else 0.0 for n_components in n_components_list],  # Remplace None par 0.0
            text=[f'{round(mean_ari_scores[(method, n_components)], 2)}' if mean_ari_scores[(method, n_components)] is not None else "N/A" for n_components in n_components_list],  # Remplace None par "N/A"
            textposition='outside',
    ...
    fig.update_layout(
        ...
        title='Mean ARI Scores for Gaussian Clustering with and without Dimensionality Reduction',
    ...
    output_path = '/home/ousmane/results/Plot_clustering_gaussian_vae.png'
