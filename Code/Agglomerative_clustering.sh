...
def cluster_and_evaluate(X, y, n_clusters):
    try:
        ...
        score = ari(y, y_pred)
        #logging.debug(f"Computed ARI score: {score} for n_clusters={n_clusters}")
        return score
    except Exception as e:
...
def main():
    ...
    #logging.debug(f"Mean ARI scores after aggregation: {mean_ari_scores}")
    ...
    fig.update_layout(
        ...
        width=3600,
        #width=4400,  # Further increase figure width
